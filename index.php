<?php
// ==================================================
// ФАЙЛ: index.php (ИСПРАВЛЕННАЯ ВЕРСИЯ)
// НАЗНАЧЕНИЕ: Главная страница
// ИСПРАВЛЕНИЯ:
//   1. Проблема 9: Добавлена запись посетителей (гостей) в таблицу visitors
//   2. Проблема 14: Год в footer изменён с 2025 на 2026
// ==================================================
require_once 'config.php';

// ==========================================================
// ✅ ИСПРАВЛЕНО (проблема 9): ЗАПИСЬ ПОСЕТИТЕЛЕЙ
// Записываем IP гостя в таблицу visitors (если не авторизован)
// ==========================================================
if (!isset($_SESSION['user_id'])) {
    $visitor_ip = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
    $today = date('Y-m-d');
    
    // Проверяем, не записан ли уже этот IP сегодня
    $stmt = $mysqli->prepare("
        SELECT id FROM visitors 
        WHERE visitor_ip = ? AND visit_date = ?
    ");
    $stmt->bind_param("ss", $visitor_ip, $today);
    $stmt->execute();
    $check = $stmt->get_result();
    
    if ($check->num_rows == 0) {
        $stmt->close();
        // Записываем нового посетителя
        $stmt = $mysqli->prepare("
            INSERT INTO visitors (visitor_ip, visit_date) 
            VALUES (?, ?)
        ");
        $stmt->bind_param("ss", $visitor_ip, $today);
        $stmt->execute();
...
<div class="container">
    ...
    <footer class="footer">
        <!-- ✅ ИСПРАВЛЕНО (проблема 14): год изменён с 2025 на 2026 -->
        <p>&copy; 2026 <?= htmlspecialchars($site_name) ?>. Все права защищены.</p>
    </footer>
</div>
</body>
</html>