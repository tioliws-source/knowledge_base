<?php
// ==================================================
// ФАЙЛ: change_password.php (БЕЗОПАСНАЯ ВЕРСИЯ)
// ==================================================
...
if ($step == 1 && $_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($target_email)) {
            ...
            $_SESSION['change_password_code'] = $code;
            $_SESSION['change_password_sent_to'] = $target_email;
            
            // ⚠️ ВАЖНО: НЕ показываем код в сообщении!
...
if ($step == 2 && $_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!$input_code) {
        ...
        } elseif ($input_code != ($_SESSION['change_password_code'] ?? '')) {
            $error = 'Неверный код';
            ...
            unset($_SESSION['change_password_code']);
