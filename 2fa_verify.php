<?php
// ==================================================
// ФАЙЛ: 2fa_verify.php (ИСПРАВЛЕННАЯ ВЕРСИЯ)
// ИСПРАВЛЕНИЯ:
//   1. Проблема 16: rand() → random_int()
//   2. Проблема 30: Добавлен rate limiting для отправки кодов
//      (макс 5 в час, защита от спама почтового ящика)
// ==================================================
...
if (isset($_POST['action']) && $_POST['action'] == 'send_code') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($hour_count >= 5) {
            $error = 'Превышен лимит отправок (5 в час). Попробуйте позже.';
        }
        
        // Проверка интервала между отправками (минимум 60 секунд)
        if (!$error && $code_data) {
...
// Получаем обновлённый код для отображения
$stmt = $mysqli->prepare("SELECT id, code, sent_to, expires_at, attempts, created_at FROM verification_codes WHERE user_id = ? AND type = '2fa' ORDER BY id DESC LIMIT 1");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$code_data = $stmt->get_result()->fetch_assoc();
...
// ✅ ИСПРАВЛЕНО (проблема 30): учитываем лимит 5 в час при отображении кнопки
$hour_count = 0;
$stmt = $mysqli->prepare("
    SELECT COUNT(*) as cnt 
    FROM verification_codes 
...
$can_resend = (!$code_data || (time() - strtotime($code_data['created_at'])) >= 60) && ($hour_count < 5);
...
<head>
    ...
    <style>
        ...
        .verify-container .rate-limit-warning { font-size: 12px; color: #856404; background: #fff3cd; padding: 8px; border-radius: 6px; margin-top: 10px; }
    </style>
</head>
<body>
<div class="container">
        ...
        <div class="user-info">
            ...
            <?php if ($hour_count >= 3): ?>
                <div class="rate-limit-warning">
                    ⚠️ Отправлено кодов за последний час: <?= $hour_count ?> из 5
                </div>
            <?php endif; ?>
        </div>
