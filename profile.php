<?php
// ==================================================
// ФАЙЛ: profile.php (ИСПРАВЛЕННАЯ ВЕРСИЯ)
// НАЗНАЧЕНИЕ: Профиль пользователя
// ИСПРАВЛЕНИЯ:
//   1. Проблема 11: Добавлена CSRF-проверка для upload_avatar
//   2. Проблема 29: mkdir с 0777 → 0755
//   3. Проблема 30: Добавлен rate limiting для отправки кодов (макс 5 в час)
//   4. rand() → random_int() для генерации кодов
//   5. Усиленная защита от XSS в имени файла аватарки
// ==================================================
...
// ==========================================================
// ✅ ИСПРАВЛЕНО (проблема 11): ЗАГРУЗКА АВАТАРКИ С CSRF
// ==========================================================
if (isset($_POST['action']) && $_POST['action'] == 'upload_avatar') {
    // CSRF-проверка
    $csrf_token = $_POST['csrf_token'] ?? $_SERVER['HTTP_X_CSRF_TOKEN'] ?? '';
    if (!verifyCsrfToken($csrf_token)) {
        echo 'Ошибка безопасности. Обновите страницу.';
        exit;
    }
    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0) {
        ...
        if (mb_strlen($original_name) > 40) {
            echo 'Название файла не должно превышать 40 символов';
            exit;
        }
        ...
        if (!in_array($mime_type, $allowed_mime)) {
            echo 'Разрешены только изображения: JPG, PNG, WebP, GIF';
            exit;
        }
        
        $max_size = 2 * 1024 * 1024;
        if ($file['size'] > $max_size) {
            echo 'Размер файла не должен превышать 2 МБ';
            exit;
        }
        ...
        if (!is_dir($avatar_dir)) {
            // ✅ ИСПРАВЛЕНО (проблема 29): права 0755
            if (!mkdir($avatar_dir, 0755, true)) {
                echo 'Не удалось создать папку для аватарок';
                exit;
            }
        }
...
// ==========================================================
// ✅ ИСПРАВЛЕНО (проблема 30): ОТПРАВКА КОДА С RATE LIMITING
// ==========================================================
if (isset($_POST['action']) && $_POST['action'] == 'send_code') {
    ...
    // ✅ ИСПРАВЛЕНО (проблема 30): Rate limiting — не более 5 кодов в час
    $stmt = $mysqli->prepare("
        SELECT COUNT(*) as cnt 
    ...
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $hour_count = $stmt->get_result()->fetch_assoc()['cnt'];
    $stmt->close();
    
    if ($hour_count >= 5) {
        echo 'Превышен лимит отправок (5 в час). Попробуйте позже.';
        exit;
    }
...
<head>
    ...
    <!-- ✅ ИСПРАВЛЕНО (проблема 23): meta csrf-token теперь в <head> -->
...
function uploadProfileAvatar() {
    ...
    // ✅ ИСПРАВЛЕНО (проблема 11): добавлен CSRF-токен
