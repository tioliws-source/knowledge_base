<?php
// ==================================================
// ФАЙЛ: edit_company.php (ИСПРАВЛЕННАЯ ВЕРСИЯ)
// НАЗНАЧЕНИЕ: Редактирование компании
// ИСПРАВЛЕНИЯ:
//   1. Проблема 20: request_delete_timestamp теперь требует CSRF
...
if ($action == 'get_company_access') {
    ...
    echo json_encode([
        ...
        'is_editor' => (int)($company['allow_editors'] ?? 0),
        'is_employee' => (int)($company['allow_employees'] ?? 0)
    ]);
    exit;
}
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] == 0) {
        ...
        if (!is_dir($logo_dir)) {
            ...
            if (!mkdir($logo_dir, 0755, true)) {
                die('Не удалось создать папку для логотипов');
            }
        }
...
// ==========================================================
// ✅ ИСПРАВЛЕНО (проблема 20): ЗАПРОС ТАЙМСТЕМПА ДЛЯ УДАЛЕНИЯ
// Теперь требует CSRF-токен!
// ==========================================================
...
function showDeleteConfirm() {
    ...
    // ✅ ИСПРАВЛЕНО (проблема 20): теперь с CSRF-токеном
    fetch('edit_company.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
